...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$("#backlistReview").click(function(){
			$("#div").empty().load("<%=request.getContextPath()%>/review/${username }/userReview?page=1");
		});
		...
		$("#upload").click(function(){
			...
			$.ajax({
				  url:"<%=request.getContextPath()%>/saveImage/review?id=${review_id}",
				  type: "POST",  
		          contentType: "json", 
		          ...
		          success: function (result) {
		        	  if (result.hasOwnProperty("code")) {
							if (result.code=="0") {
								$("#div").empty().load("<%=request.getContextPath()%>/review/${username }/userReview?page=${page}");
			...
			<tr>
				<td bgcolor="#f7f7f2">
					<table cellspacing="1" cellpadding="4" width="100%" border="0">
						<tbody>
							<tr>
								<td width="80%" height="25" bgcolor="#f1ede3"><span style="font-size: 12px;line-height: 22px;font-weight: bold;color:#CC0000;"><img height="9" src="" width="4">图片信息：请在这里发布您的投稿相关图片信息！</span></td>
								<td width="20%" bgcolor="#f1ede3" align="right">[<a href="#" class="a1" id="backlistReview">返回相关投稿列表</a>]</td>
							</tr>
						</tbody>
					</table>
				</td>
						...
						<tr id="image">
							<td align="center" id="image_0">
								<c:if test="${!empty himage_0 }">
									<img width="160px" height="150px" src="<%=request.getContextPath()%>/resources/upload/${himage_0.nname }">
								</c:if>
							</td>
							<td align="center" id="image_1">
								<c:if test="${!empty himage_1 }">
									<img  width="160px" height="150px" src="<%=request.getContextPath()%>/resources/upload/${himage_1.nname }">
								</c:if>
							</td>
							<td align="center" id="image_2">
								<c:if test="${!empty himage_2 }">
									<img  width="160px" height="150px" src="<%=request.getContextPath()%>/resources/upload/${himage_2.nname }">
								</c:if>
							</td>
							<td align="center" id="image_3">
								<c:if test="${!empty himage_3 }">
									<img   width="160px" height="150px" src="<%=request.getContextPath()%>/resources/upload/${himage_3.nname }">
								</c:if>
							</td>
						</tr>
						<tr id="name">
						...
						<tr id="image">
							<td align="center" id="image_4">
								<c:if test="${!empty himage_4 }">
									<img   width="160px" height="150px" src="<%=request.getContextPath()%>/resources/upload/${himage_4.nname }">
								</c:if>
							</td>
							<td align="center" id="image_5">
								<c:if test="${!empty himage_5 }">
									<img   width="160px" height="150px" src="<%=request.getContextPath()%>/resources/upload/${himage_5.nname }">
								</c:if>
							</td>
							<td align="center" id="image_6">
								<c:if test="${!empty himage_6 }">
									<img   width="160px" height="150px" src="<%=request.getContextPath()%>/resources/upload/${himage_6.nname }">
								</c:if>
							</td>
							<td align="center" id="image_7">
								<c:if test="${!empty himage_7 }">
									<img   width="160px" height="150px"  src="<%=request.getContextPath()%>/resources/upload/${himage_7.nname }">
								</c:if>
							</td>
						</tr>
					</table>
