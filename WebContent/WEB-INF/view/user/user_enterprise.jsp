...
<!--
	...
	function edit(id){
		$("#div").empty().load("<%=request.getContextPath()%>/enterprise/${user.username}/enterprise?id="+id+"&page="+page);
	}
	function del(id){
		$.ajax({
			url: "<%=request.getContextPath()%>/enterprise/${user.username}/delEnterprise?id="+id,
			type: "POST",
			dataType:'json',
	...
	$(document).ready(function() {
		...
		$("#addEnterprise").click(function(){
			$("#div").empty().load("<%=request.getContextPath()%>/user/${user.username}/addEnterprise");
		});
		$("#pageList").change(function(){
			var page = $("#pageList option:selected").val();
			$("#div").empty().load("<%=request.getContextPath()%>/enterprise/${user.username }/userEnterprise?page="+page);
		});
						...
						<tbody>
							<tr>
								<td width="80%" height="25" bgcolor="#f1ede3"><span
									class="zthong16"><img height="9" src="" width="4">提示：请在这里添加或编辑您的商业公司信息！</span></td>
								<td width="20%" align="right" bgcolor="#f1ede3">[<a href="#" class="a1" id="addEnterprise">点击添加公司信息</a>]&nbsp;
								</td>
							</tr>
						</tbody>
					</table>
					...
					<table cellspacing="1" cellpadding="4" width="100%" bgcolor="#cccccc" border="0">
						<tbody>
							<tr>
								...
								<td align="center" bgcolor="#eeeeee" width="20%">公司名称</td>
							...
							<c:forEach items="${enterprises }" var="enterprise" varStatus="status">
								<tr>
									...
									<td bgcolor="#ffffff" align="center">${enterprise.name }</td>
									<td bgcolor="#ffffff" align="center">${enterprise.area }</td>
									<td align="center" bgcolor="#ffffff"><c:choose>
											<c:when test="${enterprise.status == 'no'}">
									...
									<td align="center" bgcolor="#ffffff">[<a href="#" onclick="edit(${enterprise.id })">编辑</a>] [<a href="#" onclick="del(${enterprise.id })">删除</a>]
									</td>
								</tr>
							</c:forEach>
						</tbody>
